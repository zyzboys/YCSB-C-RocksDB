...
const string CoreWorkload::DBNAME_PROPERTY = "dbname";
const string CoreWorkload::DBNAME_DEFAULT = "basic";
...
void CoreWorkload::Init(const utils::Properties &p) {
  ...
  if (p.GetProperty(DBNAME_PROPERTY) == "rocksdb") {
    handleRocksdb();
  }
}
...
// rocksdb need special handle some property
void CoreWorkload::handleRocksdb() {
  // rocksdb only use one field
  field_count_ = 1;

  // reset the field_chooser_
  if (field_chooser_ != nullptr) {
    delete field_chooser_;
  }
  field_chooser_ = new UniformGenerator(0, field_count_ - 1);

  // rocksdb always read all fields
  read_all_fields_ = true;

  // rocksdb always write all fields
